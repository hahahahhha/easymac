...
object ReadWTd {
  ...
  // get depth of a mac unit
  def getMacDepth(array: Array[String]): List[Int] = {
    val inputbits = array(0).trim.split(" ")
    val res = inputbits(2).toInt :: Nil
    res
  }
  ...
  def getIn(m: Int, n: Int, myarch: List[Int],macdepth:Int): Map[List[Int], List[Int]] = {
    val len = myarch.length
    var edgesin = Map[List[Int], List[Int]]()

    var pos = Map[Int, Int]()

    val columnmax=n+m+log2Ceil(macdepth) // <=256
    var cnt = new Array[Int](256)
    for (i <- 0 until columnmax ) {
      pos += i -> 0
      cnt(i) = 0
    }
  ...
  def getOut(m: Int, n: Int, myarch: List[Int],macdepth:Int): Map[List[Int], List[Int]] = {
    val len = myarch.length
    ...
    for (i <- 0 until columnmax) {
      if (i < min) {
        ...
        pos += i -> min*macdepth
      } else if(i>=(min + abs) && i< n+ m  ){
        pos += i -> (m + n - i - 1)*macdepth
      }
      else {
        pos += i -> 0
      }
   ...
   */
  def getRes(m: Int, n: Int, myarch: List[Int],macdepth:Int): Map[Int, List[Int]] = {
    ...
    var initDots = new Array[Int](columnmax) // the initial dots in a trapezoidal shape
    var compDots = new Array[Int](columnmax) // the remaining dots after compression
    ...
    for (col <- 0 until (columnmax- 1)) {
      if (col < min) {
        initDots(col) = macdepth*(col + 1)
        rowNumber += col -> initDots(col)
      }
      else if (col >= min && col < (min + diff)) {
        initDots(col) = macdepth*min
        rowNumber += col -> initDots(col)
      } else {
        initDots(col) = Math.max(macdepth*(n + m - 1 - col),0)
        rowNumber += col -> initDots(col)
      }
    ...
    compDots(0)=initDots(0)+(1 - 2) * compressorCount(List(0, 0)) + (1 - 3) * compressorCount(List(0, 1))
    if (compDots(0) < 0 || compDots(0) > 2) {
        println("Wrong Compressors Structure! At column = " + 0 + " and res=" + compDots(0))
      }
    for (col <- 1 until columnmax-1) {

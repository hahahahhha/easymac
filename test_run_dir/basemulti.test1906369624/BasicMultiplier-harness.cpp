...
#endif
#include <iostream>
class BasicMultiplier_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    BasicMultiplier_api_t(VBasicMultiplier* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_multiplier)));
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_multiplicand)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_res)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["BasicMultiplier.reset"] = 0;
    }
#if VM_TRACE
...
static BasicMultiplier_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VBasicMultiplier* top = new VBasicMultiplier;
    std::string vcdfile = "test_run_dir/basemulti.test1906369624/BasicMultiplier.vcd";

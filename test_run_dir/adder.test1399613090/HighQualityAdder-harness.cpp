...
#endif
#include <iostream>
class HighQualityAdder_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    HighQualityAdder_api_t(VHighQualityAdder* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_inputs_0)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_inputs_1)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_outs_0)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["HighQualityAdder.reset"] = 0;
    }
#if VM_TRACE
...
static HighQualityAdder_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VHighQualityAdder* top = new VHighQualityAdder;
    std::string vcdfile = "test_run_dir/adder.test1399613090/HighQualityAdder.vcd";
...
#endif
    HighQualityAdder_api_t api(top);

...
#endif
#include <iostream>
class BasicMAC_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    BasicMAC_api_t(VBasicMAC* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorQData(&(dut->io_addend)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_multiplier)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_multiplicand)));
        sim_data.outputs.push_back(new VerilatorQData(&(dut->io_res)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["BasicMAC.reset"] = 0;
    }
#if VM_TRACE
...
static BasicMAC_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VBasicMAC* top = new VBasicMAC;
    std::string vcdfile = "test_run_dir/basemac.test351027297/BasicMAC.vcd";

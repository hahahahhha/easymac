...
#endif
#include <iostream>
class MAC_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    MAC_api_t(VMAC* _dut) {
        dut = _dut;
        main_time = 0L;
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_addend)));
        ...
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_outs)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["MAC.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VMAC* top = new VMAC;
    std::string vcdfile = "test_run_dir/systolicarray.test1911951520/MAC.vcd";
